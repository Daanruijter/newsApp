...
var fetchMethod = require("node-fetch");
var fs = require("fs");
...
router.post("/updateXMLSitemap", function (req, res) {
    var XMLData = '<?xml version="1.0" encoding="UTF-8"?>' + req.body.urlSetToSend;
    // Write updated XML string to the sitemap
    fs.writeFile("./dist/static/sitemap.xml", XMLData, function (err) {
        if (err) {
            console.log(err);
            throw err;
        }
